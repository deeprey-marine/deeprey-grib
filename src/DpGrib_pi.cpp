...
void DpGrib_pi::Internal_SetWindSpeedUnit(int unitIndex) {
  if (!m_pGribCtrlBar) return;

  GribOverlaySettings& settings = m_pGribCtrlBar->m_OverlaySettings;
  settings.UpdateGlobalUnits(GribOverlaySettings::DR_WIND_SPEED, unitIndex);

  // Trigger overlay redraw with new units
  RequestRefresh(m_parent_window);
}

void DpGrib_pi::Internal_SetDepthUnit(int unitIndex) {
  if (!m_pGribCtrlBar) return;

  GribOverlaySettings& settings = m_pGribCtrlBar->m_OverlaySettings;
  settings.UpdateGlobalUnits(GribOverlaySettings::DR_DEPTH, unitIndex);

  RequestRefresh(m_parent_window);
}

void DpGrib_pi::Internal_SetTemperatureUnit(int unitIndex) {
  if (!m_pGribCtrlBar) return;

  GribOverlaySettings& settings = m_pGribCtrlBar->m_OverlaySettings;
  settings.UpdateGlobalUnits(GribOverlaySettings::DR_TEMPERATURE, unitIndex);

  RequestRefresh(m_parent_window);
}

int DpGrib_pi::Internal_GetWindSpeedUnit() const {
  if (!m_pGribCtrlBar) return 0;

  // Get current GRIB unit for WIND and map back to DeepRey index
  const GribOverlaySettings& settings = m_pGribCtrlBar->m_OverlaySettings;
  int gribUnit = settings.Settings[GribOverlaySettings::WIND].m_Units;

  // GRIB: [0=Knots, 1=M_S, 2=MPH, 3=KPH] → DeepRey: [0=Knots, 1=Mph, 2=Km/h, 3=m/s]
  switch (gribUnit) {
    case GribOverlaySettings::KNOTS: return 0;
    case GribOverlaySettings::M_S: return 3;
    case GribOverlaySettings::MPH: return 1;
    case GribOverlaySettings::KPH: return 2;
    default: return 0;
  }
}

int DpGrib_pi::Internal_GetDepthUnit() const {
  if (!m_pGribCtrlBar) return 1; // Default: Meters

  const GribOverlaySettings& settings = m_pGribCtrlBar->m_OverlaySettings;
  int gribUnit = settings.Settings[GribOverlaySettings::WAVE].m_Units;

  // GRIB: [0=Meters, 1=Feet] → DeepRey: [0=Feet, 1=Meters, 2=Fathoms]
  switch (gribUnit) {
    case GribOverlaySettings::METERS: return 1;
    case GribOverlaySettings::FEET: return 0;
    default: return 1;
  }
}

int DpGrib_pi::Internal_GetTemperatureUnit() const {
  if (!m_pGribCtrlBar) return 0; // Default: Celsius

  const GribOverlaySettings& settings = m_pGribCtrlBar->m_OverlaySettings;
  int gribUnit = settings.Settings[GribOverlaySettings::AIR_TEMPERATURE].m_Units;

  // GRIB: [0=Celsius, 1=Fahrenheit] → DeepRey: [0=Celsius, 1=Fahrenheit, 2=Kelvin]
  switch (gribUnit) {
    case GribOverlaySettings::CELCIUS: return 0;
    case GribOverlaySettings::FAHRENHEIT: return 1;
    default: return 0;
  }
}

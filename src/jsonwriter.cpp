...
/*!
 ...
 \param step the indentation increment for new objects/arrays in number of
 spaces (default is 3). This value is ignored if you specify the
 wxJSONWRITER_TAB_INDENT flag for the \e style: the indentation increment is
 only one TAB character.
...
*/
void wxJSONWriter::SetDoubleFmtString(const char* fmt) { m_fmt = (char*)fmt; }
...
/*!
 ...
 If the value is an array or key/value map (types ARRAY and OBJECT), the
 function iterates through all JSON value object in the array/map and calls
 itself for every item in the container.
*/
int wxJSONWriter::DoWrite(wxOutputStream& os, const wxJSONValue& value,
                          const wxString* key, bool comma) {
...
int wxJSONWriter::WriteComment(wxOutputStream& os, const wxJSONValue& value,
                               bool indent) {
...
int wxJSONWriter::WriteIndent(wxOutputStream& os, int num) {
  ...
  if (!(m_style & wxJSONWRITER_STYLED) ||
      (m_style & wxJSONWRITER_NO_INDENTATION)) {
    return lastChar;
  }
...
int wxJSONWriter::WriteIntValue(wxOutputStream& os, const wxJSONValue& value) {
  ...
  snprintf(buffer, 32, "%" wxLongLongFmtSpec "d", data->m_value.m_valInt64);
#else
...
/*!
 ...
 This function uses the \n snprintf function to get the US-ASCII
 representation of the integer and simply copy it to the output stream.
 The function prepends a \b plus \b sign if the \c
 wxJSONWRITER_RECOGNIZE_UNSIGNED flag is set in the \c m_flags data member.
 Returns -1 on stream errors or ZERO if no errors.
*/
...
#if wxCHECK_VERSION(2, 9, 0) || !defined(wxJSON_USE_UNICODE)
  // this is fine for wxW 2.9 and for wxW 2.8 ANSI
  snprintf(buffer, 32, "%" wxLongLongFmtSpec "u", data->m_value.m_valUInt64);
#else
  // this is for wxW 2.8 Unicode: in order to use the cross-platform
  // format specifier, we use the wxString's sprintf() function and then
  // convert to UTF-8 before writing to the stream
...
#define MAX_BYTES_PER_ROW 20
  ...
  // if STYLED and SPLIT_STRING flags are set, the function writes 20 bytes on
  // every row the following is the counter of bytes written. the string is
  // splitted only for the special meory buffer type, not for array of INTs
  int bytesWritten = 0;
...
int wxJSONWriter::WriteSeparator(wxOutputStream& os) {
  int lastChar = '\n';
  if ((m_style & wxJSONWRITER_STYLED) &&
      !(m_style & wxJSONWRITER_NO_LINEFEEDS)) {
    os.PutC('\n');
  }
  return lastChar;
}

...
static const GLchar* particle_update_fragment_source =
    ...
    "    // Min 0.05 degrees (~5km) so particles survive at extreme zoom\n"
    "    float marginLon = max((uSpawnBounds.y - uSpawnBounds.x) * 0.3, 0.05);\n"
    "    float marginLat = max((uSpawnBounds.w - uSpawnBounds.z) * 0.3, 0.05);\n"
...
static const GLchar* ribbon_draw_vertex_source =
    ...
    "    }\n"
    "\n"
    "    // Geographic distance check: if trail points are more than 0.5 degrees\n"
    "    // apart, it's a teleport (respawn). Works at any zoom level.\n"
    "    float geoDist = length(vec2(s0.r - s1.r, s0.g - s1.g));\n"
    "    if (geoDist > 0.5) {\n"
    "        gl_Position = vec4(2.0, 2.0, 0.0, 1.0);\n"
    "        vT = 1.0; vSide = 0.0; vAlpha = 0.0;\n"
    "        return;\n"
    ...
    "\n"
    "    // Fallback: large screen jump = teleport (respawn artifact)\n"
